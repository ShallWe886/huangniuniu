...
let loginModelFlag = false; //是否已经展示了登录提示，如果已经展示了，则不重复展示
			...
			uni.request({
				...
				success: (resonpes) => {
					...
					if(resonpes.data.status == 200 ){
						...
						if(request.url == `${path}/api/login/smsLogin` || request.url == `${path}/escort/login/smsLogin` || request.url == `${path}/api/login/updateWechatByMobile` || request.url == `${path}/escort/Login/updateWechatByMobile` || request.url == `${path}/api/login/weixinLogin` || request.url == `${path}/escort/Login/weixinLogin`){
						...
						if(resonpes.data.msg == 'token不能为空') {
							if(loginModelFlag){
								return
							}
							loginModelFlag = true
					...
					}else if(resonpes.data.status == 401 || resonpes.data.msg == 'token不能为空' || resonpes.data.msg == 'token失效'){
						if(!uni.getStorageSync("hasLogin") || resonpes.data.msg == 'token失效'){
							if(loginModelFlag){
								return
							}
							loginModelFlag = true
